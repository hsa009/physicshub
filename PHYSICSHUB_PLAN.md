...
|---|---|---|---|
| 2026-06-03 | M4 | **Progress page + Explain More chat shipped (commit `eb7ae26`).** New: `src/components/ChatThread.tsx` (multi-turn, focus-on-open, Clear/Close, gold student bubbles / serif tutor bubbles, graceful offline), `src/hooks/useAllAnswers.ts` (`['answers','all',esis]` query), `src/pages/Progress.tsx` (3 stat tiles · 9 lesson cards with progress bar + verdict counts · recent activity list · empty state CTA). Modified: `api.ts` adds `ChatMessage` + `history`; `QuestionCard.tsx` wires "Explain More" to chat and closes chat on "Try again"; `worker/src/prompts.ts` `chatPrompt` formats prior turns; `worker/src/index.ts` reads optional `history`; `App.tsx` replaces `ProgressStub` with `<Progress />`. Build: 472.39 kB JS / 21.51 kB CSS (gzip 134.12 / 5.37). Worker smoke tests: `/chat` with history → 200; missing fields → 400; `/check` regression → 200. Vite dev: `/`, `/progress`, `/lesson/*` all 200. | build |
