...
|---|---|---|---|
| 2026-06-04 | M5.A | **Admin dashboard (commit `cc12dc9`).** Per user request: typing the admin password into the same Welcome box unlocks a teacher-only dashboard at `/admin`. **Worker (new deploy `d7ad790c-3951-486b-8fa5-0819b06a6b8b`):** NEW `worker/src/admin.ts` — constant-time password compare + 32-byte base64url tokens with 24h TTL. Tokens are stored in a new `ADMIN_TOKENS` KV namespace (id `53a0b75234c64a5baea16ad8c182ae59`, `expirationTtl: 86400` so stale entries auto-clean). **Why KV and not an in-memory Map:** Cloudflare Workers do not share module-level state across isolates; the first deploy of the admin auth proved this — login wrote to one isolate, the next request hit a different one and the token was 401. The KV-backed second deploy fixed it. `worker/wrangler.toml` adds the binding. `worker/src/index.ts` adds 5 bearer-checked routes — `POST /admin/login` (200/401), `GET /admin/verify`, `GET /admin/stats` (totals + verdict counts + per-lesson counts, accuracy weighted 0.5 for partial), `GET /admin/students?q=` (server-side prefix search via Supabase), `GET /admin/student/:esis` (per-student detail with every saved answer, most-recent first). All use the service-role key so a single call bypasses RLS to read the full `students` + `answers` tables. 401 clears the token on the client; 503 if Supabase is not configured on the Worker. **Client:** `src/lib/api.ts` adds `adminApi` (`login`/`verify`/`stats`/`listStudents`/`getStudent`) + token helpers (`getAdminToken`/`setAdminToken`/`clearAdminToken`) using `Authorization: Bearer <token>`. `src/hooks/useStudent.tsx` extends `StudentStatus` with `"admin"`, adds `adminSignIn(password)`, and rehydrates from KV on mount by calling `/admin/verify` (falls through to ESIS hydration on failure). `src/pages/Welcome.tsx` is **unchanged for students** — ESIS-shaped input (3-20 letters/digits/dashes, no spaces) goes straight to student sign-in. Anything else (spaces, longer than 20, special chars) is treated as a candidate password: try `adminSignIn` first, fall back to ESIS validation for a cleaner error. `src/App.tsx` adds `/admin` guarded by a new `RequireAdmin` (mirrors `RequireStudent`, checks `status === "admin"`). `src/components/Nav.tsx` shows a gold `Admin` pill + `Admin` link + `Sign out` when in admin mode; the logo links to `/admin`. Mobile menu mirrors the same. **NEW `src/pages/Admin.tsx` (3 sections):** (1) Overview — 4 stat tiles (Students, Answers, Accuracy, Most-answered lesson) using the existing eyebrow + serif numerals + gold-bordered grid; (2) Students — searchable ESIS table (200ms debounced server-side search) with columns ESIS / Answers / Accuracy / Verdicts (gold ✓ / ⚠ / ✗) / Last active — click any row to drill in; (3) Student detail — header (ESIS in gold, close button), 4 stat tiles (First seen, Last active, Answers, Accuracy), per-lesson breakdown list, then a full answer list with `<details>` blocks showing the student answer + AI feedback. All numbers reuse the existing Noir Luxe classes (`eyebrow`, `section-title`, `btn-soft`, feature-item borders, `gold-line`). **Build:** 540.12 kB JS (+13.66 kB) / 30.90 kB CSS (+1.37 kB) — bundle `index-qwrSdBYI.js`. Worker `tsc --noEmit` clean; 0 TS errors in the client. End-to-end verified via curl: login → 200, verify → 200, stats returns 7 students + 9 answers + 66.6% accuracy, students list returns 7 rows, student/569377 returns the full 9-answer detail. Pages auto-deployed; new bundle live on `revisionforyou.pages.dev`. | build |
| 2026-06-04 | M5.6 | **Question images from docx truth + lightbox (commit `c7c0b66`).** Per user feedback: question prompts were showing only text — the embedded diagrams from the docx were never wired up. **Source of truth:** built `scripts/verify-question-images.mjs` (adm-zip, walks `word/document.xml` paragraphs, groups `<a:blip r:embed="rIdN"/>` images by `N.` question markers, mirrors `extract-images.mjs`'s `.jpeg → .jpg` rename, and writes `src/data/question-images.json` from the document itself — no hand-curated substitutes). **Findings:** 59/60 questions have an image in the docx (only q60 is text-only). Previous file had 49 mapped + 11 null hand-curated; docx truth has 10 of those 11 "null" questions actually containing an image (q2, q13, q16, q18, q20, q33, q40, q41, q42, q50). After the `.jpeg → .jpg` fix all 59 image files exist on disk. **UI:** `src/components/ImageLightbox.tsx` (NEW — full-screen `<dialog>` modal with Esc / backdrop / × close, gold square close button, optional caption, reduced-motion safe fade). `src/components/QuestionCard.tsx` resolves `getQuestionImage(id)` and renders a clickable gold-bordered frame above the answer area (lazy-load, `Click to enlarge` hint, focus-visible ring); click opens the lightbox. `src/styles/index.css` adds `.question-image` (gold border-l accent, bg-subtle, fade-in keyframe), `.image-lightbox` + `::backdrop` blur, `.image-lightbox-frame/img/close/caption`, all behind a `prefers-reduced-motion` block. `package.json` adds `npm run verify:question-images`. **Build:** 526.46 kB JS (+0.14 kB) / 29.53 kB CSS (+2.71 kB) — bundle `index-O-xlKPoN.js`. | build |
| 2026-06-03 | M5.9 | **Learning Path (commit `6a768b9`).** Per user request: students now move through the 9 lessons in a linear path ordered by module number (Module 6 → 7 → 8 → 9 → 10). **Open path — no lockout:** every lesson is always clickable; the path is the recommended order, not a strict gate. **Definition of "completed":** every question in the lesson has at least one saved answer (right or wrong) — same as the existing `LessonCard.complete` flag. **New files:** `src/data/learning-path.ts` (`learningPath: readonly string[]` + `getPathPosition`/`getNextLessonId`/`getPreviousLessonId`/`getPathLessons`/`getQuestionCount`/`assertPathValid`) and `src/hooks/useLearningPath.ts` (wraps `useAnsweredCounts`; returns ordered path entries augmented with `isCompleted`/`answeredCount`/`position`/`total` + `currentLessonId` + per-lesson next/prev getters; `isLessonComplete()` helper for non-hook callers). **Modified:** `src/types/index.ts` adds derived `questionCount: number` to `LessonWithSlides`; `src/data/lessons.ts` enriches `allLessons` with `questionCount = questionIds.length`; `src/components/LessonCard.tsx` gains optional `pathPosition` + `pathTotal` + `isUpNext` props (eyebrow shows `3 of 9 · Module 7`, gold `✦ Up next` badge + `ring-1 ring-gold/60` highlight on the next-incomplete card); `src/components/QuestionsPager.tsx` adds `isComplete` + `nextLesson` + `onNextLesson` props — on the last question, the `Next →` button transforms into a btn-primary `Continue to [Next Lesson] →` CTA when the lesson is complete and a next lesson exists, and renders an inline `Nine *Lessons* Done` celebration block (`Review any lesson` + `View Progress` CTAs) when complete with no next; `src/pages/Home.tsx` adds a `Your Path` hero (eyebrow + `3 of 9 *Lessons*` + gold progress bar), filters AND sorts the grid by path, numbers each card 1–9; `src/pages/LessonView.tsx` wires everything up — `isComplete` + `nextLesson` flow into the pager, a small `✓ Completed` pill appears in the hero when done, and a `Next Lesson →` header link is added alongside `Back to slides` + `Practice Mode`. **Edge cases:** all 9 lessons stay navigable from any card (consistent with "open" locking); search still works on the path-ordered grid; `Skip to questions` + `Practice Mode` still work on completed lessons; question IDs stay as `q1`–`q60` from the docx (the path order differs from the docx order, so e.g. `Projectile Motion` (3rd in path) still shows `q8 → q13`); navigating to the next lesson resets the slide position automatically (`useSlidePosition` is keyed by `lesson.id`); TanStack Query invalidation on `useSaveAnswer` keeps the path progress reactive. Build 526.32 kB JS (+5.34 kB) / 26.82 kB CSS (+0.54 kB), 0 TS errors. | build |
| 2026-06-03 | Hotfix | **Paged questions + hide-until-asked (commit `a8c03a7`).** Per user request: questions section is now hidden until the student explicitly asks for it. Slides and questions are rendered **mutually exclusively** in `LessonView` based on a new `showQuestions` state (default `false`). Triggers: `Go to Questions` on the last slide, `Skip to questions` link, OR finishing the last slide (`onComplete`). A new `QuestionsPager.tsx` shows ONE question at a time with explicit **Previous** (btn-soft) and **Next →** (btn-ghost) buttons — **no auto-scroll** between cards, just like the slide pager. The pager also has a header with `← Back to slides` (returns to the slide view at the saved position) and `✦ Practice Mode` (navigates to `/lesson/:id/practice`), plus a gold progress bar that mirrors the slide pager. When `showQuestions` flips to `true` the page smoothly scrolls to the questions section. Build 520.98 kB JS (+2.17 kB) / 26.28 kB CSS, 0 TS errors. | build |
| 2026-06-03 | Hotfix | **Lesson links resolve to canonical slug id (commit `11f6a66`).** Reported by user on the live site: every Home grid card and every Progress per-lesson CTA showed "Not found / No such Lesson". Root cause: `LessonCard.encodeLessonPath()` fell back to `/lesson/${module}::${name}` when the lesson had no `id` field — and the Home page passes legacy lessons (from `questions.json`, no `id`). `Progress.tsx` had the same bug AND used the removed `/lesson/:module/:name` route. **Fix:** added `lessonIdByModuleName` map + `getLessonIdByModuleName()` in `src/data/lessons.ts`; both call sites now look up the canonical id (e.g. `gravitation`) by `(module, name)`. Old `::`-joined string is only used as a last-resort fallback if a lesson is missing from `lessons.json`. Build 518.81 kB JS / 26.28 kB CSS. | build |
...
| 2026-06-03 | M5.1 | **Image extraction (commit `14dd8e2`).** `scripts/extract-images.mjs` (adm-zip + sharp) extracts 78 embedded images from the docx's `word/media/` directory. Filters entries < 1 KB (inline spacers/icons). SVGs copied verbatim. PNGs re-encoded at `compressionLevel: 9` + palette quantization. JPGs re-encoded with mozjpeg at quality 82. Rasters whose long side > 1200 px are downsized (without enlargement). Idempotent. New devDeps: `adm-zip ^0.5.16`, `sharp ^0.33.5`. New npm script: `extract:images`. Result: **78 images (39 PNG / 30 JPG / 9 SVG), 13.43 MB raw → 4.86 MB compressed (63.8% savings), 12 resized, 0 failed.** Largest 567 KB, smallest 1.2 KB, avg 64 KB. `public/images/lessons/image-manifest.json` is the source of truth for M5.2 (image-overrides.json) and M5.6 (question-images.json). Vite copies `public/` to `dist/` unchanged — verified after `npm run build` (all 78 files present in `dist/images/lessons/`). | build |
| 2026-06-03 | M5.2 | **Lesson schema + hand-curated content (commit `81cab2d`).** Discovered the docx has **zero explanatory text** between questions (just dotted answer lines) — so all 9 lessons needed hand-written slide content based on Grade 11 physics curriculum + the 60 question prompts. **Types:** `src/types/index.ts` adds `Slide`, `ImageRef`, `FormulaRef`, `LessonWithSlides`, `LessonsFile`, `QuestionImageMap`, `QuestionImagesFile`. **Data:** `src/data/lessons.json` — 9 lessons × 3 slides = **27 slides total**. Each slide has title + 200-370 char markdown body. 25/27 have a `FormulaRef` (the 2 without are pure-conceptual). All 27 use a **unique** image from the 78-image manifest. Per lesson: `id` (URL slug), `module`, `name`, `accentWord` (italic-gold on cards), `description`, `slides[]`, `questionIds[]`. **Access:** `src/data/lessons.ts` is the new canonical module — exports `allLessons`, `lessonById`, `getLessonById`, `getSlideById`, `questionImageMap`, `getQuestionImage`, plus re-exports `questionBank`/`getQuestion`/`getQuestionsForLesson` for backward compat. `src/data/bank.ts` is now a thin re-export — `getLesson` re-added to keep `LessonView` working. **Override surfaces:** `src/data/image-overrides.json` (lesson_id → slide_id → image; future re-build hook) and `src/data/question-images.json` (60 question_id → image; 49 mapped, 11 null for pure-text questions). **Build:** 498.96 kB JS (+26.7 kB for new data + types), 22.31 kB CSS unchanged, 0 TS errors. All 27 slide image URLs return 200 on the live Pages site. | build |
| 2026-06-03 | M5.3 | **Paged lesson UI (commit `c20cdfc`).** Replaces the auto-loading AI explanation panel with a proper 3-slide pager driven by the new `lessons.json`. **New components:** `src/lib/format.tsx` (50 lines — `renderBody` for `**bold**`, `renderAccentTitle` for the gold accent word), `src/hooks/useSlidePosition.ts` (localStorage-backed position, key `physicshub.slidePosition`, safe for SSR / private-mode / quota), `src/components/Slide.tsx` (eyebrow + serif title + body + optional formula card + optional image with caption; lazy-loads; uses slide id as React key so each swap re-runs the fade-in), `src/components/LessonPager.tsx` (progress bar with `role=progressbar` + ARIA, Back/Next with btn-soft/btn-ghost, last slide swaps in `Go to Questions` (btn-primary) + `Practice Mode` (btn-ghost, alert placeholder until M5.5), keyboard `←/→` ignored inside inputs, `Start over` + `Skip to questions` secondary links). **Refactored:** `src/pages/LessonView.tsx` — new route `useParams<{lessonId: string}>`, `getLessonById`, `NotFound` extracted, hero shows the accent word via `renderAccentTitle`, smooth scroll to the question section on `onComplete`, Practice button scroll + alert placeholder. **Routes:** `src/App.tsx` swaps `/lesson/:module/:name` for `/lesson/:lessonId` (clean v2 break, no back-compat). `src/components/LessonCard.tsx` links to `/lesson/:id`. **Styles:** `src/styles/index.css` adds `.slide-title` (clamp 1.8–2.6 rem), `.slide-body` (1.0625 rem / 1.75 lh, `**strong**` gold-bright), `.formula-card` (gold border-l + bg-card), `.formula-expression` (serif 1.3 rem), and a `.slide-fade-in` keyframe. New `prefers-reduced-motion` block disables slide-fade-in / slow-rotate / btnPulse / fade-up-stagger. **Build:** 502.42 kB JS (+3.5 kB) / 23.33 kB CSS (+1 kB); 0 TS errors; Vite warns about >500 kB chunk but build passes. | build |
| 2026-06-03 | M5.5 | **Practice Questions (commit `cb38289`).** AI-generated 3-question sets per lesson. Per-lesson, ephemeral, and off the saved-progress path. Reuses `/check` for verdict/feedback. **Worker:** `worker/src/prompts.ts` adds `practicePrompt` (system prompt: lesson, module, slide titles, 2 existing questions for style reference; asks for exactly 3 NEW questions, mixed conceptual+numerical, strict JSON shape, anti-image rule). `worker/src/parse.ts` adds `parsePracticeResponse` (same JSON extraction as `parseExplainResponse` — fenced / brace-slice / numbered-list fallback; type normalised to `conceptual` or `numerical` via number-keyword heuristic). `worker/src/mock.ts` adds `mockPractice` (3 plausible placeholder questions). `worker/src/index.ts` adds `POST /practice` + `handlePractice` (reuses `callWithRotation`, returns 400 if no usable questions). **Client:** `src/lib/api.ts` adds `PracticeType`, `PracticeQuestion`, `PracticeRequest`, `PracticeResponse`, `api.generatePractice()`. **UI:** `src/components/PracticeQuestionCard.tsx` (NEW — one ephemeral question; mirrors `QuestionCard`'s style; eyebrow with type + index; serif prompt; textarea; Check My Answer (btn-primary) + Skip (btn-soft); inline `VerdictBadge` + feedback block; Try again / Next Practice buttons; `<details>` block reveals the student's submitted answer after grading). `src/components/PracticePanel.tsx` (NEW — initial CTA "Generate Practice Questions" (btn-primary with ✦); loading spinner + 3 skeleton cards; active header with visible/total counter + "Regenerate Set" (btn-ghost); skipped cards hidden, not destroyed; "Set complete" panel after all skipped/answered). `src/pages/PracticePage.tsx` (NEW — the route; renders the `PracticePanel` inside Noir Luxe chrome; title "Practice [AccentWord]"; top-left "Back to [Lesson]" link). **Routes:** `src/App.tsx` adds `/lesson/:lessonId/practice` guarded by `RequireStudent`. `src/pages/LessonView.tsx` now uses `useNavigate`; the "Practice Mode" button in the LessonPager AND in the questions section header both navigate here. **Verification:** 518.66 kB JS (+10.6 kB) / 26.28 kB CSS (+0.5 kB); 0 TS errors; worker `tsc --noEmit` clean; `wrangler deploy` v f298cc10-bc3e-440c-bd78-c17ff6fa7e14; **the AI provider came back online** — `/practice` returned 3 real AI-generated Universal Gravitation questions (1 conceptual + 2 numerical) on first call, 3 different questions on second call; live `/lesson/gravitation/practice` returns 200. | build |
| 2026-06-03 | M5.4 | **Ask-AI-Anything drawer (commit `f0408db`).** Slide-aware AI tutor panel. ✦ button (bottom-right desktop, centered mobile) opens a drawer; AI gets the current slide's title + body + formula and answers as a Grade 11 tutor that guides rather than solves. **Worker:** `worker/src/prompts.ts` adds `AskSlideInput` + `askSlidePrompt` (system prompt bakes in lesson, module, slide body, formula, tutor role, rules: guide not solve, 1-3 paragraphs max ~90 words, no emojis, off-topic redirects). `worker/src/index.ts` adds `POST /ask` + `handleAsk`, reuses `callWithRotation` so the same Groq/OpenRouter + 60s per-key cooldown applies. **Client:** `src/lib/api.ts` adds `AskRequest`, `AskResponse`, `api.askSlide()`. **UI:** `src/components/AskDrawer.tsx` (NEW — right-docked on desktop 420 px, bottom-sheet 85 vh on mobile; header shows lesson accent word + current slide title; messages scrollable, Enter to send / Shift+Enter newline / Esc closes; per-lesson React state; `aria-live="polite"` + `role="dialog"`). `src/components/AskToggleButton.tsx` (NEW — pill button, rotates to ✕ when open, hides on desktop when drawer open, pulse animation for unread messages, reduced-motion safe). **Refactor:** `src/components/LessonPager.tsx` adds `onSlideChange` callback. `src/pages/LessonView.tsx` tracks `currentSlide` in state, mounts button + drawer. **Styles:** `src/styles/index.css` adds `.ask-drawer` (fixed, transform-translate off-screen by default, `--open` slides in), `.ask-backdrop` (mobile only), `.ask-toggle` (pill, hover/--open/--has-messages variants), `askPulse` keyframe. **Build:** 508.04 kB JS (+5.6 kB) / 25.75 kB CSS (+2.4 kB); 0 TS errors; worker `tsc --noEmit` clean; `wrangler deploy` v1b280422-40a7-4812-a9f4-5088938e2fd0; `/ask` returns the expected "all keys exhausted" today (Groq key now 403 — appears revoked; OpenRouter free 429). Endpoint is wired correctly; once a fresh key is in, the drawer works end-to-end. | build |
